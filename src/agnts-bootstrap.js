...
export function buildAgntsManagedBlock(env = process.env) {
  ...
  return ensureTrailingNewline([
    ...
    'Use this order:',
    `1. Run \`${readinessCommand}\` and read the JSON result.`,
    '2. If `clusterWatchReady` is `true`, run the cluster watch report.',
    `3. If the watch triggers, run \`${drilldownCommand}\` with the reported behavioral cluster id.`,
    `4. Fall back to \`${degradationCommand}\` only for explicit ranking-only needs or if the readiness gate says cluster watch is blocked.`,
    '',
